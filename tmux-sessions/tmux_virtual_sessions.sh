...
while read -a line; do
    ...
    echo "    * Name/Description: ${line[0]} - Port: ${line[1]} - Login: ${line[2]} - Host: ${line[3]}"

    if [[ line_counter -eq 0 ]];then
        session_command="ssh -p ${line[1]} -l ${line[2]} ${line[3]}"
    else
        split_list+=( split-pane ssh -l ${line[2]} -p ${line[1]} ${line[3]} ';' )
    fi
